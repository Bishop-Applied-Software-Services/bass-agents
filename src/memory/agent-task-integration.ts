...
import { MemoryEntry, MemoryQueryFilters } from './types';
 ...
 */
export async function populateMemoryContext(
  task: AgentTask,
  ...
  try {
    ...
    console.warn(
      `Warning: Failed to populate memory context for task ${task.task_id}: ${error instanceof Error ? error.message : String(error)}`
    );
 ...
 */
function prioritizeByEvidenceType(entries: MemoryEntry[]): MemoryEntry[] {
  // Separate entries by evidence type
  const withCodeOrArtifact: MemoryEntry[] = [];
  const withOtherEvidence: MemoryEntry[] = [];

  for (const entry of entries) {
    const hasCodeOrArtifact = entry.evidence.some(
