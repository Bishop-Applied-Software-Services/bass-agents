...
exports.main = main;
const memory_adapter_1 = require("../memory/memory-adapter");
 ...
 * Handle init command
 */
async function handleInit(adapter, parsed) {
 ...
 * Handle list command
 */
async function handleList(adapter, parsed) {
    const project = parsed.args[0] || 'global';
    // Build filters from options
    const filters = {};
    if (parsed.options.section) {
        filters.section = [parsed.options.section];
    }
    if (parsed.options.kind) {
        filters.kind = [parsed.options.kind];
    }
    if (parsed.options.scope) {
        filters.scope = [parsed.options.scope];
    }
    if (parsed.options.subject) {
        filters.subject = [parsed.options.subject];
    }
    if (parsed.options.status) {
        filters.status = [parsed.options.status];
    }
    if (parsed.options['min-confidence']) {
        filters.minConfidence = parseFloat(parsed.options['min-confidence']);
    }
 ...
 * Handle show command
 */
async function handleShow(adapter, parsed) {
    if (parsed.args.length === 0) {
    ...
    const project = parsed.options.project || 'global';
 ...
 */
async function handleQuery(adapter, parsed) {
    if (parsed.args.length === 0) {
    ...
    const matchingEntries = entries.filter(entry => entry.summary.toLowerCase().includes(searchText.toLowerCase()) ||
 ...
 */
async function handleCompact(adapter, parsed) {
    const project = parsed.args[0] || 'global';
 ...
 */
async function handleValidateEvidence(adapter, parsed) {
    const project = parsed.args[0] || 'global';
 ...
 */
async function handleCheckFreshness(adapter, parsed) {
    const project = parsed.args[0] || 'global';
 ...
 */
async function handleSyncContext(adapter, parsed) {
    if (parsed.args.length === 0) {
 ...
 */
async function handleExport(adapter, parsed) {
 ...
 */
async function handleImport(adapter, parsed) {
    if (parsed.args.length < 2) {
    ...
    const conflictStrategy = parsed.options['conflict-strategy'] || 'skip';
    ...
    const report = await adapter.import(project, inputPath, conflictStrategy);
    console.log(`\nImport Report:`);
    console.log(`  Total entries: ${report.totalEntries}`);
 ...
 */
async function handleDashboard(adapter, parsed) {
    const project = parsed.args[0] || 'global';
    const showAll = parsed.options.all === true;
    const rangeOption = parsed.options.range || 'all';
    ...
    if (webMode) {
        await displayWebDashboard(project, showAll, parsed.options.out);
        return;
    }
 ...
 */
function parseDateRange(rangeOption) {
    const now = new Date();
    switch (rangeOption) {
        case '7d': {
 ...
 */
async function handleStats(adapter, parsed) {
    const project = parsed.args[0] || 'global';
    const showAll = parsed.options.all === true;
    const rangeOption = parsed.options.range || 'all';
    const bypassCache = parsed.options['no-cache'] === true;
 ...
 */
async function displayDashboard(adapter, project, dateRange, bypassCache, showAll) {
 ...
 */
async function displayWebDashboard(project, showAll, outPath) {
    const workspaceRoot = getWorkspaceRoot();
    ...
    try {
        const output = (0, child_process_1.execFileSync)('python3', args, {
            cwd: workspaceRoot,
            encoding: 'utf-8',
