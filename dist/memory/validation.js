 ...
 */
const VALID_SECTIONS = ['decisions', 'state', 'observations', 'learnings'];
/**
...
function validateMemoryEntry(entry) {
    ...
    if (entry.section && !VALID_SECTIONS.includes(entry.section)) {
        errors.push(`Field "section" must be one of: ${VALID_SECTIONS.join(', ')}. Got: "${entry.section}"`);
    }
    // Validate kind enum
    if (entry.kind && !VALID_KINDS.includes(entry.kind)) {
        errors.push(`Field "kind" must be one of: ${VALID_KINDS.join(', ')}. Got: "${entry.kind}"`);
    }
    // Validate scope pattern
    if (entry.scope && !SCOPE_PATTERN.test(entry.scope)) {
        errors.push(`Field "scope" must match pattern: repo | service:<name> | org | customer | environment:<prod|staging>. Got: "${entry.scope}"`);
    }
    // Validate summary length
    if (entry.summary && entry.summary.length > MAX_SUMMARY_LENGTH) {
        errors.push(`Field "summary" must not exceed ${MAX_SUMMARY_LENGTH} characters. Got: ${entry.summary.length} characters`);
    }
    // Validate content length
    if (entry.content && entry.content.length > MAX_CONTENT_LENGTH) {
        errors.push(`Field "content" must not exceed ${MAX_CONTENT_LENGTH} characters. Got: ${entry.content.length} characters`);
    }
    // Validate confidence range
    if (entry.confidence !== undefined && entry.confidence !== null) {
        if (entry.confidence < MIN_CONFIDENCE || entry.confidence > MAX_CONFIDENCE) {
            errors.push(`Field "confidence" must be between ${MIN_CONFIDENCE} and ${MAX_CONFIDENCE}. Got: ${entry.confidence}`);
        }
    }
    ...
    if (entry.status && !VALID_STATUSES.includes(entry.status)) {
        errors.push(`Field "status" must be one of: ${VALID_STATUSES.join(', ')}. Got: "${entry.status}"`);
    }
    // Add warning for low confidence
    if (entry.confidence !== undefined && entry.confidence < 0.5) {
        warnings.push(`Low confidence score: ${entry.confidence}. Consider providing stronger evidence or marking as draft.`);
    }
    return {
...
function validateEvidence(evidence, index) {
    ...
    if (evidence.type && !validTypes.includes(evidence.type)) {
        errors.push(`Evidence at index ${index} has invalid type. Must be one of: ${validTypes.join(', ')}. Got: "${evidence.type}"`);
    }
    return errors;
}
/**
