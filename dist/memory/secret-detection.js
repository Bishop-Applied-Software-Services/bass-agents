...
exports.validateNoSecrets = validateNoSecrets;
/**
 ...
 */
function detectSecretsInText(text, fieldName) {
    const errors = [];
    for (const pattern of SECRET_PATTERNS) {
        if (pattern.pattern.test(text)) {
            errors.push(`Detected ${pattern.description} in ${fieldName}. ` +
 ...
 * @returns Detection result with errors if secrets found
 */
function detectSecretsInEvidence(evidence) {
    const errors = [];
 ...
 */
function validateNoSecrets(entry) {
