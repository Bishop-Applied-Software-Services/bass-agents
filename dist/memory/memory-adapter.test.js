...
(0, vitest_1.describe)('MemoryAdapter', () => {
    const testWorkspaceRoot = path.join(__dirname, '../../test-workspace');
    const testProject = 'test-project';
    let adapter;
    (0, vitest_1.beforeEach)(async () => {
        adapter = new memory_adapter_1.MemoryAdapter(testWorkspaceRoot);
    ...
    (0, vitest_1.describe)('init', () => {
        (0, vitest_1.it)('should initialize memory storage for a project', async () => {
            await adapter.init(testProject);
            const memoryPath = path.join(testWorkspaceRoot, 'ai-memory', testProject);
            const configPath = path.join(memoryPath, '.config.json');
            // Check directory exists
            (0, vitest_1.expect)(fs.existsSync(memoryPath)).toBe(true);
            // Check config file exists
            (0, vitest_1.expect)(fs.existsSync(configPath)).toBe(true);
            // Check config content
            const configContent = await fs.promises.readFile(configPath, 'utf-8');
            const config = JSON.parse(configContent);
            (0, vitest_1.expect)(config.project).toBe(testProject);
            (0, vitest_1.expect)(config.version).toBe('1.0.0');
            (0, vitest_1.expect)(config.created_at).toBeDefined();
        });
    });
    (0, vitest_1.describe)('create', () => {
        (0, vitest_1.it)('should create a new memory entry', async () => {
            const entry = {
                section: 'decisions',
                kind: 'decision',
        ...
        (0, vitest_1.it)('should reject entry with validation errors', async () => {
            ...
            await (0, vitest_1.expect)(adapter.create(testProject, invalidEntry)).rejects.toThrow(/validation failed/i);
        });
        (0, vitest_1.it)('should reject entry with secrets', async () => {
            ...
            await (0, vitest_1.expect)(adapter.create(testProject, entryWithSecret)).rejects.toThrow(/secret detection failed/i);
        });
        (0, vitest_1.it)('should reject duplicate entries', async () => {
            const entry = {
                section: 'decisions',
                kind: 'decision',
                subject: 'auth-service.authentication',
            ...
            await (0, vitest_1.expect)(adapter.create(testProject, entry)).rejects.toThrow(/duplicate entry detected/i);
        });
    });
    (0, vitest_1.describe)('supersede', () => {
        (0, vitest_1.it)('should supersede an existing entry', async () => {
            const originalEntry = {
                section: 'decisions',
                kind: 'decision',
                subject: 'auth-service.authentication',
            ...
            const replacementId = await adapter.supersede(testProject, originalId, replacementEntry);
            (0, vitest_1.expect)(replacementId).toBeDefined();
            (0, vitest_1.expect)(replacementId).not.toBe(originalId);
        });
        (0, vitest_1.it)('should throw error for non-existent target', async () => {
            const replacementEntry = {
                section: 'decisions',
                kind: 'decision',
                subject: 'test',
            ...
            await (0, vitest_1.expect)(adapter.supersede(testProject, 'bd-nonexistent', replacementEntry)).rejects.toThrow(/not found/i);
        });
    });
    (0, vitest_1.describe)('deprecate', () => {
        (0, vitest_1.it)('should deprecate an existing entry', async () => {
            const entry = {
                section: 'decisions',
                kind: 'decision',
                subject: 'auth-service.authentication',
            ...
            await (0, vitest_1.expect)(adapter.deprecate(testProject, entryId)).resolves.not.toThrow();
        });
        (0, vitest_1.it)('should throw error for non-existent target', async () => {
            await (0, vitest_1.expect)(adapter.deprecate(testProject, 'bd-nonexistent')).rejects.toThrow(/not found/i);
        });
    });
    (0, vitest_1.describe)('query', () => {
        (0, vitest_1.it)('should return empty array for uninitialized project', async () => {
            const results = await adapter.query('nonexistent-project');
            (0, vitest_1.expect)(results).toEqual([]);
        });
        (0, vitest_1.it)('should query entries with section filter', async () => {
            const decision = {
                section: 'decisions',
                kind: 'decision',
                subject: 'auth',
            ...
            (0, vitest_1.expect)(results[0].section).toBe('decisions');
        });
        (0, vitest_1.it)('should apply default filters (status=active, confidence>=0.6)', async () => {
            ...
            const results = await adapter.query(testProject);
            (0, vitest_1.expect)(results.length).toBe(1);
            (0, vitest_1.expect)(results[0].confidence).toBeGreaterThanOrEqual(0.6);
        });
        (0, vitest_1.it)('should expand scope hierarchy', async () => {
            const repoEntry = {
                section: 'decisions',
                kind: 'decision',
                subject: 'repo-level',
            ...
            (0, vitest_1.expect)(scopes).toContain('repo');
            (0, vitest_1.expect)(scopes).toContain('service:auth');
        });
        (0, vitest_1.it)('should return summary-only when requested', async () => {
            const entry = {
                section: 'decisions',
                kind: 'decision',
                subject: 'test',
            ...
            (0, vitest_1.expect)(results.length).toBe(1);
            (0, vitest_1.expect)(results[0].summary).toBe('Test summary');
            (0, vitest_1.expect)(results[0].content).toBe('');
            (0, vitest_1.expect)(results[0].evidence).toEqual([]);
        });
    });
    (0, vitest_1.describe)('get', () => {
        (0, vitest_1.it)('should retrieve a single entry by ID', async () => {
            const entry = {
                section: 'decisions',
                kind: 'decision',
                subject: 'test',
            ...
            (0, vitest_1.expect)(retrieved.summary).toBe('Test entry');
        });
    ...
    (0, vitest_1.describe)('getRelated', () => {
        (0, vitest_1.it)('should retrieve related entries', async () => {
            const entry1 = {
                section: 'decisions',
                kind: 'decision',
                subject: 'test1',
            ...
            (0, vitest_1.expect)(related.length).toBe(2);
            const relatedIds = related.map(r => r.id);
            (0, vitest_1.expect)(relatedIds).toContain(id1);
            (0, vitest_1.expect)(relatedIds).toContain(id2);
        });
        (0, vitest_1.it)('should return empty array for entry with no related entries', async () => {
            const entry = {
                section: 'decisions',
                kind: 'decision',
                subject: 'test',
    ...
    (0, vitest_1.describe)('compact', () => {
        (0, vitest_1.it)('should return compaction report with dry-run', async () => {
            ...
            (0, vitest_1.expect)(report.project).toBe(testProject);
            (0, vitest_1.expect)(report.dryRun).toBe(true);
            (0, vitest_1.expect)(report.totalEntries).toBeGreaterThan(0);
            (0, vitest_1.expect)(report.supersededEntries).toBeGreaterThan(0);
            (0, vitest_1.expect)(report.timestamp).toBeDefined();
        });
        (0, vitest_1.it)('should handle compaction when bd compact is not available', async () => {
            ...
            (0, vitest_1.expect)(report).toBeDefined();
            (0, vitest_1.expect)(report.project).toBe(testProject);
            (0, vitest_1.expect)(report.dryRun).toBe(false);
        });
    });
    (0, vitest_1.describe)('export', () => {
        (0, vitest_1.it)('should export memory entries to JSONL format', async () => {
            // Create test entries
            const entry1 = {
                section: 'decisions',
                kind: 'decision',
            ...
            // Verify file exists
            (0, vitest_1.expect)(fs.existsSync(outputPath)).toBe(true);
            // Verify content
            const content = await fs.promises.readFile(outputPath, 'utf-8');
            const lines = content.split('\n').filter(line => line.trim());
            (0, vitest_1.expect)(lines.length).toBeGreaterThanOrEqual(2);
            // Parse first line
            const exported = JSON.parse(lines[0]);
            (0, vitest_1.expect)(exported.section).toBeDefined();
            (0, vitest_1.expect)(exported.summary).toBeDefined();
            (0, vitest_1.expect)(exported.confidence).toBeDefined();
        });
        (0, vitest_1.it)('should export with filters', async () => {
            ...
            for (const line of lines) {
                ...
                (0, vitest_1.expect)(entry.section).toBe('decisions');
                (0, vitest_1.expect)(entry.confidence).toBeGreaterThanOrEqual(0.8);
            }
        });
    });
    (0, vitest_1.describe)('import', () => {
        (0, vitest_1.it)('should import memory entries from JSONL format', async () => {
            // Create JSONL file
            const entries = [
                {
                    id: 'bd-test001',
            ...
            await fs.promises.writeFile(inputPath, entries.map(e => JSON.stringify(e)).join('\n'), 'utf-8');
            // Import
            const report = await adapter.import(testProject, inputPath, 'skip');
            (0, vitest_1.expect)(report).toBeDefined();
            (0, vitest_1.expect)(report.project).toBe(testProject);
            (0, vitest_1.expect)(report.totalEntries).toBe(1);
            (0, vitest_1.expect)(report.successCount).toBeGreaterThan(0);
            (0, vitest_1.expect)(report.errorCount).toBe(0);
        });
        (0, vitest_1.it)('should handle conflicts with skip strategy', async () => {
            // Create an entry
            const entry = {
                section: 'decisions',
                kind: 'decision',
            ...
            const report = await adapter.import(testProject, inputPath, 'skip');
            (0, vitest_1.expect)(report.skipCount).toBe(1);
            (0, vitest_1.expect)(report.conflicts.length).toBe(1);
            (0, vitest_1.expect)(report.conflicts[0].resolution).toBe('skipped');
        });
        (0, vitest_1.it)('should handle invalid entries gracefully', async () => {
            // Create JSONL with invalid entry
            const inputPath = path.join(testWorkspaceRoot, 'import-invalid.jsonl');
            await fs.promises.writeFile(inputPath, 'invalid json\n{"valid": "but incomplete"}', 'utf-8');
            // Import should not throw
            const report = await adapter.import(testProject, inputPath, 'skip');
            (0, vitest_1.expect)(report.errorCount).toBeGreaterThan(0);
            (0, vitest_1.expect)(report.errors.length).toBeGreaterThan(0);
        });
    });
    (0, vitest_1.describe)('syncContext', () => {
        (0, vitest_1.it)('should sync high-confidence entries to ai-context/', async () => {
            ...
            (0, vitest_1.expect)(fs.existsSync(contextDir)).toBe(true);
            // Check for generated file
            const files = await fs.promises.readdir(contextDir);
            (0, vitest_1.expect)(files.length).toBeGreaterThan(0);
            // Verify content
            const content = await fs.promises.readFile(path.join(contextDir, files[0]), 'utf-8');
            (0, vitest_1.expect)(content).toContain('auth-service');
            (0, vitest_1.expect)(content).toContain('Use JWT for authentication');
            (0, vitest_1.expect)(content).toContain('**Confidence:** 0.9');
        });
        (0, vitest_1.it)('should filter out low-confidence entries', async () => {
            ...
            if (fs.existsSync(contextDir)) {
                ...
                for (const file of files) {
                    const content = await fs.promises.readFile(path.join(contextDir, file), 'utf-8');
                    (0, vitest_1.expect)(content).not.toContain('**Confidence:** 0.5');
                }
            }
        });
        (0, vitest_1.it)('should group entries by subject', async () => {
            ...
            const content = await fs.promises.readFile(path.join(contextDir, authFile), 'utf-8');
            (0, vitest_1.expect)(content).toContain('Use JWT');
            (0, vitest_1.expect)(content).toContain('Auth requirements');
        });
    });
    (0, vitest_1.describe)('workspace boundary enforcement', () => {
        (0, vitest_1.it)('should reject init with path outside workspace', async () => {
            // Try to initialize with path traversal
            const maliciousProject = '../../../etc/passwd';
            await (0, vitest_1.expect)(adapter.init(maliciousProject)).rejects.toThrow(/workspace boundary violation/i);
        });
        (0, vitest_1.it)('should reject export with path outside workspace', async () => {
            await adapter.init(testProject);
            // Try to export to path outside workspace
            const maliciousPath = '/tmp/outside-workspace.jsonl';
            await (0, vitest_1.expect)(adapter.export(testProject, maliciousPath)).rejects.toThrow(/workspace boundary violation/i);
        });
        (0, vitest_1.it)('should reject import with path outside workspace', async () => {
            await adapter.init(testProject);
            // Try to import from path outside workspace
            const maliciousPath = '/etc/passwd';
            await (0, vitest_1.expect)(adapter.import(testProject, maliciousPath)).rejects.toThrow(/workspace boundary violation/i);
        });
        (0, vitest_1.it)('should reject syncContext with malicious project name', async () => {
            ...
            await (0, vitest_1.expect)(adapter.syncContext(maliciousProject)).rejects.toThrow(/workspace boundary violation/i);
        });
        (0, vitest_1.it)('should allow operations within workspace', async () => {
            ...
            await (0, vitest_1.expect)(adapter.export(testProject, exportPath)).resolves.not.toThrow();
            // Import within workspace
            await (0, vitest_1.expect)(adapter.import(testProject, exportPath)).resolves.toBeDefined();
            // Sync context within workspace
            await (0, vitest_1.expect)(adapter.syncContext(testProject)).resolves.not.toThrow();
        });
    });
});
