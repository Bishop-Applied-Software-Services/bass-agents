...
exports.applyMemoryUpdates = applyMemoryUpdates;
 ...
 */
async function applyMemoryUpdates(result, project, workspaceRoot, agentIdentifier) {
    const applyResult = {
        totalUpdates: 0,
        successCount: 0,
        errorCount: 0,
    ...
    for (let i = 0; i < result.memory_updates.length; i++) {
        ...
        try {
            ...
            console.error(`Error applying memory update ${i + 1}/${applyResult.totalUpdates} ` +
...
function validateMemoryUpdate(update) {
    ...
    if (update.operation === 'create') {
        ...
        const validationResult = (0, validation_1.validateMemoryEntry)(entryInput);
...
//# sourceMappingURL=agent-result-integration.js.map